...
class CheckLangBehavior extends Behavior {
    ...
    private function checkLanguage() {
        ...
        if(C('IS_ADMIN') && C('ADMIN_LANG_ON')) {
            if(isset($_GET['switch'])){
                $langSet = $_GET['switch'];// url中设置了语言变量
                cookie('think_admin_language',$langSet,3600);
			}elseif(cookie('think_admin_language')){// 获取上次用户的选择
                $langSet = cookie('think_admin_language');
            }else{ // liuxun add 默认语言
                $langSet = C('ADMIN_DEFAULT_LANG');
            }
        }
        // 定义当前语言
        define('LANG_SET',strtolower($langSet));
        //file_put_contents('log.log',  LANG_SET . "\r\n", FILE_APPEND);
