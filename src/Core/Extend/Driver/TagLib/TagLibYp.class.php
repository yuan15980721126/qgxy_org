...
class TagLibYp extends TagLib
{
	protected $tags   =  array(
        ...
        'list'=>array('attr'=>'name,field,limit,order,catid,thumb,posid,where,sql,key,page,mod,id,ids,status,parentid','level'=>3),
		'subcat'=>array('attr'=>'catid,bottom,type,self,key,id','level'=>3),
		'catpos' => array('attr'=>'catid,space','close'=>0),
		'nav' => array('attr'=>'catid,bcid,level,id,class,home,enhomefont,bottom','close'=>0),
		...
		'blockdetail' => array('attr'=>'blockid,pos,key,id','close'=>1),
    );
	...
	public function _pre($attr,$content) {
		...
		$r = M($m)->where("catid=$catid and id<$id and $langWhere")->order("id DESC")->find();
	...
	public function _next($attr,$content) {
		...
		$r = M($m)->where("catid=$catid and id>$id and $langWhere")->order("id ASC")->find();
	...
	public function _blockdetail($attr,$content) {
		$tag    = $this->parseXmlAttr($attr,'blockdetail');
		$id = !empty($tag['id'])?$tag['id']:'r';  //定义数据查询的结果存放变量
		$key    = !empty($tag['key'])?$tag['key']:'i';
		$pos   = !empty($tag['pos'])? $tag['pos'] : '';
		$mod    = isset($tag['mod'])?$tag['mod']:'2';
		...
		if($pos)$where .=" and pos='$pos' ";
		if($blockid)$where .=" and id=$blockid ";
		$sql = "M('Block')->where(\"{$where}\")->find()";
		//下面拼接输出语句
		$parsestr  = '';
		$parsestr .= '<?php $' . "$id = $sql; if ($" . $id . '):?>';
		$parsestr .= $content;//解析在blockdetail标签中的内容
		$parsestr .= '<?php endif;?>';
		return  $parsestr;
	}
	...
	public function _subcat($attr,$content) {
		...
		$bottom = !empty($tag['bottom'])?$tag['bottom']:'';
		...
		$parsestr .= 'if(1=="' . $bottom . '" && $'.$id.'[\'isfootermenu\']==0){ continue; }';
	...
	public function _catpos($attr) {
		...
		$space		= !empty($tag['space']) ? $tag['space'] : '';
 		if(is_numeric($tag['catid'])){
			...
			$parsestr .= '$icnt = 0;foreach($arrparentid as $cid):';
			$parsestr .= '$parsestr[] = \'<a \'.(count($arrparentid)==(++$icnt)?\'class="last"\':"").\' href="\'.$Categorys[$cid][\'url\'].\'">\'.$Categorys[$cid][\'catname\'].\'</a>\';?>';
			$parsestr .= '<?php endforeach;echo implode("<em>'.$space.'</em>",$parsestr);?>';
			//echo $parsestr;exit;
			return  $parsestr;
		}
	...
	public function _list($attr,$content) {
			...
			{   //根据用户输入的值拼接查询条件
				...
				if($tag['name'] && isset($tag['parentid'])) {
					$allModule = F('Module');
					$allMods = F('Mod');
					if(isset($allMods[ucfirst($tag['name'])])) {
						$currMod = $allModule[$allMods[ucfirst($tag['name'])]];
						if($currMod['parent_moduleid'] > 0) {
							$pMod = $allModule[$currMod['parent_moduleid']];
							$where .=  '  AND `' . $pMod['name'] . '_id` ='.$tag['parentid'];
						}
