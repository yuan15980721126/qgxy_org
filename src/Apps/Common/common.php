...
function delattach($map=''){
		...
		foreach((array)$att as $key=> $r){
			$aids[]=$r['aid'];
			@unlink($_SERVER['DOCUMENT_ROOT'].__ROOT__.$r['filepath']);
		}
...
function thumb2($f, $tw=300, $th=300 ,$type=2, $color='255,255,255', $nopic = 'nopic.jpg',$t=''){
	if(strstr($f,'://')) return $f;
	if(empty($f)) return thumb2(__ROOT__.'/Public/Images/'.$nopic,$tw,$th);
	$f= '.'.str_replace(__ROOT__,'',$f);
	...
	$color = explode(',', $color);
	$image = new ThinkImage(THINKIMAGE_GD, $f, $color);
	$image->thumb($tw,$th,$type)->save($t);
...
// add by jzp
function hasCatalogField($moduleid) {
	...
	{
		if(!$agent) $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
        $res =0; //默认pc
        $res = (strpos($agent, 'iphone')) ? 1 : $res;
        $res =  (strpos($agent, 'android')) ? 1 : $res;
        $res = (strpos($agent, 'ipad')) ? 2 : $res;
        if (!$res) {
          $res =  (strpos($agent, 'mobile')) ? 1 : $res;
        }
        return $res;
	}
	function pics_to_arr($str){
    	 $arr=explode(':::',$str);
         foreach ($arr as $k => $v) {
        	$res[]=explode('|', $v);
        }
    ...
    /*添加日志*/
    function logs($username,$note,$error=0){

        $data=array();
        $data['userid']=$_SESSION['userid'] ? $_SESSION['userid'] : 0;
 ...
 * @param int $return_type 默认值为0，0/不为0则拼接返回，1/*秒，2/*分*秒，3/*时*分*秒/，4/*日*时*分*秒，5/*月*日*时*分*秒，6/*年*月*日*时*分*秒,7/*年
...
{
    if ($return_type < 0 || $return_type > 7) {
        return false;
    }
    ...
    if (($return_type == 0 || $return_type == 6 || $return_type == 7) && $remainder_seconds - 31536000 > 0) {
