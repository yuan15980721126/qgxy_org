...
define('THINKIMAGE_THUMB_FILLED_FOREVER',       7); //常量，标识缩略图缩放后填充类型（原图尺寸小于目标生成尺寸，先等比放大后，再生成）
...
class ThinkImage{
     ...
     */
    public function __construct($type = THINKIMAGE_GD, $imgname = null, $bgColor = array(255, 255, 255)){
        /* 判断调用库的类型 */
        switch ($type) {
            case THINKIMAGE_GD:
                $class = 'ImageGd';
        ...
        $this->img = new $class($imgname, $bgColor);
    }
     ...
     * @return Object          当前图片处理库对象
     */
    public function thumb($width, $height, $type = THINKIMAGE_THUMB_FILLED){
