	...
	public function doUpdateurl()
    {
		...
		if($moduleid<=0 && $catids[0] <= 0){
			...
			if(!isset($_SESSION['moduleids'][$doid])){
					...
					if($count){
						$list = $dao->field('id,catid,url,title')->where($where)->limit($start . ',' . $pagesize)->select();		 
						foreach($list as $r) {
...
//				print_R($name);
		...
		if($moduleid == 19 || $moduleid == 20 || $moduleid == 21){
			$name[$c]='createtime';
		}else{
			$name[$c]='thumb';
		}
...
//			print_R($data);
				foreach ($line_list AS $line)
				{
                    ...
                    $line = eval('return '.iconv('gbk','utf-8',var_export($line,true)).';');
					...
					for ($i = 0; $i < $len; $i++)
					{
                        $char = $line[$i];
                        $field_name = $name[$i];
                        $arr[$field_name] = addslashes(trim($char));
...
//					if(!empty($arr['grade'])){
//						$arr['grade']=intval($arr['grade']);
//					}
					if(isset($arr['gettime']) && !empty($arr['gettime'])){
	                    if(!is_time($arr['gettime'])){
	                        $this->error('有效期日期格式错误');
	                    }else{
	                        $arr['gettime']=strtotime($arr['gettime']);
	                    }
	                }
	                if(isset($arr['realtime']) && !empty($arr['realtime'])){
	                    if(!is_time($arr['realtime'])){
	                        $this->error('有效期日期格式错误');
	                    }else{
	                        $arr['realtime']=strtotime($arr['realtime']);
	                    }
	                }
	                if(isset($arr['effecttime']) && !empty($arr['effecttime'])){
	                    if(!is_time($arr['effecttime'])){
	                        $this->error('有效期日期格式错误');
	                    }else{
	                        $arr['effecttime']=strtotime($arr['effecttime']);
	                    }
	                }
	                if(isset($arr['createtime']) && !empty($arr['createtime'])){
	                    if(!is_time($arr['createtime'])){
	                        $this->error('有效期日期格式错误');
	                    }else{
	                        $arr['createtime']=strtotime($arr['createtime']);
	                    }
	                }
	                if(count($arr) >0){
	...
	public function down()
	{
		...
		$moduleid_arr = [19, 20, 21, 22]; //单位模型 siuloong 20190516

		$namearr=implode(",",$name);
		if(!in_array($moduleid, $moduleid_arr)){
            $namearr.=',缩略图(图片路径)';
        }
