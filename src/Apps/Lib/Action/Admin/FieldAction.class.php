...
class FieldAction extends AdminbaseAction {
	...
	public function _before_add(){
        ...
        // add by jzp
        if(!hasCatalogField($this->moduleid))
        {
            $this->assign('show_catalog', true);
        }
	}
	...
	function delete() {
        ...
        //to-do: 判断是否为副栏目， 如果是， 执行删除关联表操作， 否则只删除对应字段。
        if($r['type'] == 'catalog') {
            $this->dao->execute("DROP TABLE IF EXISTS `{$tablename}_catalog`");
        } else {
	...
	public function get_tablesql($info,$do){
		...
		switch($fieldtype) {
			...
			// add by jzp 
			case 'catalog':
				$attachtable = $tablename . '_catalog';
				$indexfield = strtolower($this->module[$moduleid]['name'] . '_id');
				//这里与输入的field关联。
				$linkfield = $field;  //strtolower('catalog_id');
				$sql[] = "CREATE TABLE IF NOT EXISTS `$attachtable` ( `$indexfield` int(11) unsigned NOT NULL,  `$linkfield` int(11) unsigned NOT NULL, primary key(`$indexfield`, `$linkfield`)) ENGINE=MyISAM DEFAULT CHARSET=utf8";
                $sql[] = "CREATE INDEX `idx_{$attachtable}_{$indexfield}` on `$attachtable`(`$indexfield`)";
                $sql[] = "CREATE INDEX `idx_{$attachtable}_{$linkfield}`  on `$attachtable`(`$linkfield`)";
			break;
		}
		return $sql;
	}
