...
class ConfigAction extends AdminbaseAction {
	...
	public function index() {
		...
		foreach($config as $key=>$r) {
			...
			//if($r['groupid']==1)$this->user_config[$r['varname']]=$r; 默认版本语句
				//add by lin 按语言版本添加变量
			if($r['groupid']==1){
				if(APP_LANG){
	...
	public function mail() {
		...
		$prefix = C('DB_PREFIX');
		$sql="select * from `{$prefix}config` where `varname`='mail_ssl'";
		$result = mysql_query($sql);
		$row = mysql_fetch_array($result);
		if(empty($row['varname'])){
		
			mysql_query("INSERT INTO `{$prefix}config` VALUES ('mail_ssl', 'SSL验证', '4', '1', '0')");
		}
