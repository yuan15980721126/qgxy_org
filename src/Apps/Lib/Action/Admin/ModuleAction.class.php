...
class ModuleAction extends AdminbaseAction {
    ...
    {	
		...
		foreach ((array)$this->module as $rw){
			if($rw['type']==1 && $rw['status']==1 && $rw['id'] > 1)  $data['module'][$rw['id']] = $rw;
        }
	...
	function insert(){	
		...
		$parentModuleId = intval( $_POST['parent_moduleid'] );
		$parentModuleField = '';
		if($parentModuleId > 0) {
			$parentModule = $this->module[$parentModuleId];
			if($parentModule) {
			 	$colsName = strtolower( $parentModule['name'] . '_id' );
				$parentModuleField = "`{$colsName}` int(8) unsigned NOT NULL DEFAULT '0',";
			}
		}
